...
def joker_score(cards):
    counts = Counter(cards)
    joker_count = counts.get("d", 0)
    del counts["d"]
    counts = sorted([a for a in counts.values()], reverse=True)
    print(counts, joker_count)
    if len(counts) == 0:
        counts.append(0)
    if len(counts) == 1:
        counts.append(0)
...
if __name__ == "__main__":
    ...
    print(document)
    winnings = main_part_1(doc=document)
    print("Result Part 1: ", winnings)
